...
for kk=1:size(MM,1)
    ...
    hold off
    if kk == 1
        title({'RBF kinetic model','',['T = ',num2str(round(T-273)),' $^\circ C$, P = ',num2str(round(P)),' bar,  $\rho_f$ = ',num2str(round(rho)),'~$kg/m^3$']})
    else
    ...
    hold off
    if kk == 1
        title({'First principle kinetic model','',['T = ',num2str(round(T-273)),' $^\circ C$, P = ',num2str(round(P)),' bar,  $\rho_f$ = ',num2str(round(rho)),'~$kg/m^3$']})
    else
        title(['T = ',num2str(round(T-273)),' $^\circ C$, P = ',num2str(round(P)),' bar,  $\rho_f$ = ',num2str(round(rho)),'~$kg/m^3$'])
    end
    colorbar
...
%exportgraphics(figure(1), ['RBFvsFP.png'], "Resolution",300);