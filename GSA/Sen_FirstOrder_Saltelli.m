...
y_A = f(A, varargin{:}); y_A = y_A(:);
y_B = f(B, varargin{:}); y_B = y_B(:);
[N, k] = size(A);
...
for j = 1:k
    ...
    A_Bj = A;
    A_Bj(:, j) = B(:, j);
    y_ABj = f(A_Bj, varargin{:}); y_ABj = y_ABj(:);

    % First-order (Saltelli 2010): pair yB with y(A_Bj)
    sen_vector(j) = (mean(y_B .* y_ABj) - f_0^2) / VarY;
